...
<button class="btn-sm btn-outline" onclick="editClientModalV2('${c.id}')">✏️ Редактировать</button>
...
function editClientModalV2(id){
 ...
 openModal('<div style="position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:99;display:flex;align-items:center;justify-content:center;padding:16px" onclick="if(event.target===this)closeModal()">'+
  ...
  '<div class="row2" style="margin-top:12px"><button class="btn" style="background:#f3f4f6;color:#374151" onclick="closeModal()">Отмена</button><button class="btn btn-green" onclick="saveEditClientV2(\''+c.id+'\')">Сохранить</button></div></div></div>');
}
...
function addClientModal(){
 openModal('<div style="position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:99;display:flex;align-items:center;justify-content:center;padding:16px" onclick="if(event.target===this)closeModal()">'+
  ...
  '<div class="row2" style="margin-top:12px"><button class="btn" style="background:#f3f4f6;color:#374151" onclick="closeModal()">Отмена</button><button class="btn btn-green" onclick="saveNewClientV2()">Сохранить</button></div></div></div>');
}
...
<button class="btn-sm btn-red" onclick="cancelOrder(${o.id})">Отменить</button>
</div></div>`).join(''):'<div class="muted">Просроченных заказов нет 🎉</div>'}</div>`;
}
function rescheduleOrder(id){
 const o=byId(id); if(!o) return;
 const nd=prompt('Новая дата (ГГГГ-ММ-ДД):', o.date); if(!nd) return;
 const nt1=prompt('Новое время начала (ЧЧ:ММ):', o.t1); if(!nt1) return;
 const nt2=prompt('Новое время окончания (ЧЧ:ММ):', o.t2); if(!nt2) return;

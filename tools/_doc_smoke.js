...
const total=o=>{var ex=(o.extras||[]).reduce((s,e)=>s+(+e.price||0)*(+e.qty||1),0);var m=o.matsInTotal?(o.materials||[]).reduce((s,x)=>s+(+x.price||0),0):0;if((o.works||[]).length)return (o.works||[]).filter(x=>!x.skipped).reduce((s,x)=>s+(+x.price||0)*(+x.qty||1),0)+ex+m;return (+o.price||0)+ex+m;};
...
let fail=0;
for(const n of ['docWorksRows','docWorksRowsDone','docMatsRowsPlan','docMatsRowsFact','docExtrasRows','docExtrasTotal','docActTotal','docPerm','docOrg','docCreatedTs','docCompletedTs','docSignHtml','docText','docPrint','docShare','docCopy','docDownload','saveOrg','docEstimate','docAct','docShell','docActionsHtml','docHeadHtml','docReopen']){
  const f=grab(n);
...
const o={id:5,client:'Тест',worker:'Влад',diagnostician:null,by:'Влад',phone:'+7 900',address:'ул. Ленина 1',
  ...
  extras:[{name:'Демонтаж',qty:2,price:300,unit:'pcs'}],
...
console.log('EST total:',money(total(o)),'(ожид. 7 200 ₽):',money(total(o))==='7\u00A0200 ₽'||money(total(o))==='7 200 ₽');
console.log('ACT total:',money(docActTotal(o)),'(ожид. 6 300 ₽):',money(docActTotal(o))==='6\u00A0300 ₽'||money(docActTotal(o))==='6 300 ₽');
// extras обязаны попадать в строки документов, иначе ИТОГО не сходится с суммой строк
console.log('EST extras rows:',docExtrasRows(o).includes('Демонтаж'),'| extras total 600:',docExtrasTotal(o)===600);
